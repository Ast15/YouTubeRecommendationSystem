 ...
 * 
 * 09/2012: Tien D. Le updated logic in computeHITS method.
 *
 */
public class HITS {
	...
	public HITS ( WebGraph graph ) {
		...
		for(int i=1; i<numLinks+1; i++) { //Start from 0... Tien change to start from 1
			hubScores.put(new Integer(i),new Double(1));
			authorityScores.put(new Integer(i),new Double(1));
			//Tien: initialize hubS, autS for every node=1
		}
		computeHITS();
	}
...
//					hubScore += ((Double)(authorityScores.get((Integer)(outIter.next())))).doubleValue();//Tien updated
//				}
...
//				}
	...
	private void normalize(){
		...
		Double authorityScore = (Double)(authorityScores.get(new Integer(i)));
		Double hubScore = (Double)(hubScores.get(new Integer(i)));
		double autValue=authorityScore.doubleValue();
		double hubValue=hubScore.doubleValue();
		sumAut+=autValue;
		sumHub+=hubValue;
		}
		for (int i=1;i<graph.numNodes()+1;i++){
			...
			double autValue=authorityScore.doubleValue();
			double hubValue=hubScore.doubleValue();
			double newAutValue=autValue/sumAut;
			double newHubValue=hubValue/sumHub;
			authorityScores.put(new Integer(i),new Double(newAutValue));
			hubScores.put(new Integer(i),new Double(newHubValue));
		}
